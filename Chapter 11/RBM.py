...
class RBM(object):
    ...
    def MakeGraph(self, inputUser):
        ...
        maxWeight = -4.0 * \
            np.sqrt(6.0 / (self.hiddenDimensions + self.visibleDimensions))
        self.weights = tf.Variable(tf.random.uniform(
            [self.visibleDimensions, self.hiddenDimensions], minval=-maxWeight, maxval=maxWeight), tf.float32, name='weights')
        ...
        hProb0 = tf.nn.sigmoid(
            tf.matmul(inputUser, self.weights) + self.hiddenBias)
        # sample from all of the distributions
        hSample = tf.nn.relu(
            tf.sign(hProb0 - tf.random.uniform(tf.shape(hProb0))))
        # stitch it together
        ...
        weightUpdate = self.weights.assign_add(
            self.learningRate * (forward - backward))
        ...
        visibleBiasUpdate = self.visibleBias.assign_add(
            self.learningRate * tf.reduce_mean(inputUser - vProb, 0))
    ...
    def MakeVisible(self, feed):
        visible = tf.nn.sigmoid(
            tf.matmul(feed, tf.transpose(self.weights)) + self.visibleBias)
        return visible
